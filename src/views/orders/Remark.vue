<template>
  <div class="remark">
    ...
    <div class="view-body">
      ...
      <button @click="clickSubmit" class="btn-submit">确定</button>
    </div>
  </div>
</template>
...
export default {
  ...
  data() {
    return {
      ...
      switchItem: [
        ...
        { select: false, name: "加醋" },
        { select: false, name: "少油" },
        { select: false, name: "不要汤" },
      ],
      text: "",
    };
  },
  methods: {
    handleRadioItem(item) {
      this.radioItem.forEach((ele) => (ele.select = false));
      item.select = true;
    },
    clickSubmit() {
      let selectItems = "";
